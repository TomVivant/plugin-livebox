...
class livebox extends eqLogic {
	...
	function getCookiesInfo() {
		if ( ! defined($this->_cookies) )
		{
			...
			if( $json === false ) {
				...
				curl_setopt($session, CURLOPT_URL, 'https://'.$this->getConfiguration('ip').'/ws');
				curl_setopt($session, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($session, CURLOPT_COOKIESESSION, true);
				curl_setopt($session, CURLOPT_COOKIEJAR, COOKIE_FILE);
				curl_setopt($session, CURLOPT_COOKIEFILE, COOKIE_FILE);
	...
	function getContext($paramInternet) {
		return array("X-Context: ".$this->_contextID,
	...
	function getPage($page, $option = array()) {
		...
		foreach ($listpage as $pageuri => $param) {
			...
			log::add('livebox','debug',$page.' => url '.$this->_http_mode.'://'.$this->getConfiguration('ip').'/'.$pageuri);
			log::add('livebox','debug',$page.' => param '.$param);
			$session = curl_init();

			curl_setopt($session, CURLOPT_HTTPHEADER, $this->getContext($param));
			curl_setopt($session, CURLOPT_POSTFIELDS, $param); 
			curl_setopt($session, CURLOPT_URL, 'https://'.$this->getConfiguration('ip').'/'.$pageuri);
			curl_setopt($session, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($session, CURLOPT_COOKIESESSION, true);
			curl_setopt($session, CURLOPT_COOKIEJAR, COOKIE_FILE);
			curl_setopt($session, CURLOPT_COOKIEFILE, COOKIE_FILE);
			...
			curl_setopt($session, CURLOPT_SSL_VERIFYHOST, false);
			$json = curl_exec ($session);
			log::add('livebox','debug',$page.' => content '.$json);
			if ( is_object($statuscmd) )
			{
				if ( $json === false ) {
					if ($statuscmd->execCmd() != 0) {
					...
					log::add('livebox','error',__('La livebox ne repond pas.',__FILE__)." ".$this->getName()." : ".curl_error ($session));
					return false;
				}
				if (is_object($statuscmd) && $statuscmd->execCmd() != 1) {
					$statuscmd->setCollectDate('');
